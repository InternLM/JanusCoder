...
class Runner:
    def __init__(
        ...
        **kwargs
        ...
        self.model = get_model(self.model_name, base_url, api_key, **kwargs)
    ...
    def run(self, task: Task, output_framework: Framework, mode: Mode, execution_range: List, save_dir: str, max_workers: int = 20, stop_if_error=False) -> int:
        assert execution_range is not None, "execution_range must be provided"
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            execution_tasks = [(task, str(i), output_framework, mode, save_dir) for i in execution_range]
    ...
    def run_generation(self, args: Tuple) -> int:
        task, web_number, output_framework, mode, save_dir = args
        ...
        # save_dir = f"results/{task.value}/{self.model_filename}/"
        save_name = f"{web_number}"
    ...
    def run_edit(self, args: Tuple) -> int:
        ...
        # save_dir = f"results/{task.value}/{self.model_filename}/"
        save_name = f"{web_number}_{mode.value}"
        if self.check_files_exist(save_dir=save_dir, save_name=save_name, output_formats=output_formats):
            return web_number
