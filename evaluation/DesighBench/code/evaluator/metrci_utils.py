...
def screenshot(code_path: str, save_path: str):
    """
    使用Playwright进行截图
    """
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # 等待页面渲染完成（如果有交互按钮等需要等待）
            page.wait_for_timeout(2000)  # 等待2秒，你可以根据实际情况调整
            ...
            print(f"异常 in screenshot (utils.py): {e}")
            raise e
        finally:
            page.close()
            context.close()
...
class PWDriver:
    def __init__(self, browser_name='firefox', url=None, file=None, string=None, headless=None, viewport=None):
        self.browser_name = browser_name  # 'firefox' | 'chromium' | 'webkit'
        self.headless = True if headless is None else bool(headless)
        self.url = url
        self.file = file
        self.string = string
        self.viewport = viewport  # e.g. {"width":1920,"height":1080} or None
        self.browser = None
        self.context = None
        self.page = None
        self.init_url = None
    ...
    def _create_driver(self):
        ...
        self.context = self.browser.new_context(viewport=self.viewport)  # None=跟随系统窗口大小
        self.page = self.context.new_page()

        if self.file:
            p = Path.cwd() / self.file
            self.init_url = p.as_uri()  # file:///...
            # self.page.goto(self.init_url, wait_until="networkidle")
            self.page.goto(self.init_url, wait_until="domcontentloaded", timeout=60_000)
        elif self.string:
            self.page.set_content(self.string, wait_until="networkidle")
        elif self.url:
            self.init_url = self.url
        ...
        if self.viewport is None:
            self.page.set_viewport_size({"width": 1920, "height": 3000})

    def take_screenshot(self, filename):
        # full_page=True 对应 Firefox 的 save_full_page_screenshot
...
def validate_issue(res_path, config_path):
    ...
    generated_issue = set(generated_issue)
    # print(generated_issue, issues, len(issues.intersection(generated_issue)) / len(issues.union(generated_issue)))
...
def render_ui(code_path, save_path, frame_work):
    ...
    if frame_work == "vanilla":
        ...
        try:
            ...
            # web_driver = PWDriver(browser_name='firefox', url=None, file=code_path, string=None, headless=True)
            web_driver.take_screenshot(filename=save_path)
            web_driver.quit()
        except Exception as e:
            print(e)
