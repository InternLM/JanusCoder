...
def take_screenshot_single(browser:Browser, url, output_file, output_bbox=False):
    ...
    try:          
        ...
        with open(url, 'r', encoding='utf-8') as f:
            html_content = f.read()
        page.set_content(html_content, timeout=30000, wait_until='domcontentloaded')
        all_images_loaded(page)

        # Take the screenshot
        page.screenshot(path=output_file, full_page=True, animations="disabled", timeout=30000)
        if output_bbox:
...
if __name__ == "__main__":
    ...
    assert not (Path(args.input).is_dir() and Path(args.output).is_dir()), "Input and output should both be dir or single file !"    
